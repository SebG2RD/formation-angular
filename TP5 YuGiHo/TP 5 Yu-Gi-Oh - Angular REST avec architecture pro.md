...
>
> - **`CardApiService`** : _uniquement_ les appels HTTP (aucun état).
...
> 🔑 **`HttpParams`** construit la _query string_ proprement (encodage automatique). On évite la concaténation manuelle `?num=20&offset=...`.
...
<form class="filtres" (submit)="$event.preventDefault(); rechercher()">
  ...
  <select (change)="filtrerAttribut($any($event.target).value)">
    ...
    @for (a of filters.attributes; track a) {
...
<div class="grille">
  ...
  <article class="carte">
    ...
    <h3>{{ c.name }}</h3>
    <button
      (click)="collection.ajouter(c)"
      [disabled]="collection.estDansDeck(c.id)"
    >
...
<div class="detail">
  ...
  <div>
    <h1>{{ c.name }}</h1>
    <p>
      <strong>Type :</strong> {{ c.type }} @if (c.attribute) { · {{ c.attribute
      }} }
    </p>
    @if (c.atk != null) {
    <p><strong>ATK/DEF :</strong> {{ c.atk }} / {{ c.def }}</p>
    } @if (c.level) {
    <p><strong>Niveau :</strong> {{ c.level }} ⭐</p>
    } @if (c.archetype) {
    <p><strong>Archétype :</strong> {{ c.archetype }}</p>
    }
    <p class="desc">{{ c.desc }}</p>

    @if (c.card_prices?.[0]) {
    <p>
      <strong>Prix (Cardmarket) :</strong> {{
      c.card_prices?.[0]?.cardmarket_price }} €
    </p>
    }
...
<ul class="deck">
  ...
  <li>
    ...
    <button
      (click)="collection.ajouter(entry.card)"
      [disabled]="entry.quantite >= 3"
    >
      +
    </button>
